...
export const ProductModal = () => {
    const { products, selectedProduct } = useAppSelector((state) => state.cart);
    ...
    useEffect(() => {
        if (
            products.some((item: any) => item.product.id === selectedProduct.id)
        ) {
            setQuantity(
                products.find(
    ...
    return (
        <div className='flex flex-col items-center gap-4 p-2.5'>
            {selectedProduct.image ? (
                <img src={selectedProduct.image} className='h-42 rounded-lg' />
            ) : (
